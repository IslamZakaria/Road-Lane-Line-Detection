...
def list_images(img, cols= 2, rows= 5, cmap=None, operation = 'Figure'):
    """
...
def pixel_points(y1, y2, line):
    """
    Convert the slope and intercept for each line to pixel points.
        Parameters:
            y1: the line's starting y.
            y2: the line's end y.
            line: The slope and intercept of the line.
    """
    if line is None:#No line
        return None
    ...
    for x in range(width):
        for y in range(height):
            ...
            if grad_mag < low_threshold:
                suppressd_img[y, x] = 0
            elif high_threshold > grad_mag and grad_mag  >= low_threshold:
                ids[y, x] = 1
            else:
                ids[y, x] = 2
...
def detect_edges_canny(img, low_threshold= 50, high_threshold= 150):
    ...
    magnitude, slope = cv2.cartToPolar(gx, gy, angleInDegrees=True)
...
def frame_processor(image):
    ...
    result       = draw_lines_connected(image, lane_lines(image, hough))
    return result
...
def test_proccess():
    ...
    list_images(list(map(RGB_color_selection, test_images)), 2, 4, None, 'Select White And Yellow Lane FROM RGB')
    # 2 convert to HSV
    list_images(list(map(convert_rgb_to_hsv, test_images)), 2, 4, None, 'Convert RGB To HSV')
    # 4 Threshold HSV for Yellow and White (combine the two results together)
    list_images(list(map(HSV_color_selection, test_images)), 2, 4, None, 'combine White And Yellow Lane\n FROM HSV & RGB')
    ...
    list_images(gray_images, 2, 4, None, 'Convert Image To Grey')
    # 6 Apply noise remove (gaussian) to the masked gray image
    ...
    list_images(edge_detected_images, 2, 4, None, 'Apply Canny')
    # 8 mask the image using the canny detector output
    ...
    for image, lines in zip(test_images, hough_lines):
        lane_images.append(draw_lines_connected(image, lane_lines(image, lines)))
